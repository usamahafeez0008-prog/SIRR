...
class VehicleInformationController extends GetxController {
  ...
  Future<void> getVehicleTye() async {
    ...
    await FireStoreUtils.getDriverProfile(FireStoreUtils.getCurrentUid()).then((
      value,
    ) async {
      if (value != null) {
          ...
          if (driverModel.value.zoneIds != null) {
            if (zoneList.isNotEmpty) {
              for (var element in zoneList) {
                if (driverModel.value.zoneIds?.contains(
                      element.id.toString(),
                    ) ==
                    true) {
                  zoneString.value =
  ...
  void setVehicleDetails() {
    ...
    for (int index = 0; index < selectedPrices.length; index++) {
      rates.add(
        RateModel(
          acPerKmRate: selectedPrices[index].acCharge ?? '',
          nonAcPerKmRate: selectedPrices[index].nonAcCharge ?? '',
          perKmRate: selectedPrices[index].kmCharge ?? '',
